...
def train(df, num_timesteps, seed, policy=CnnPolicy):
...
def main():
    ...
    logger.configure(dir='./logs/ppo_train')
    # logger.configure()
    df = pd.read_hdf('./data/poloniex_30m.hf',key='train')
    train(df, num_timesteps=args.num_timesteps, seed=args.seed)
    logger.configure(dir='./logs/ppo_test')
